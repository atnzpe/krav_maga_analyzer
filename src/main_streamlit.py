...
# Isso é crucial para que as importações de src.utils e src.video_analyzer funcionem
...
def streamlit_main():
    ...
    with col1:
        ...
        uploaded_file_aluno = st.file_uploader(
            ...
            type=["mp4", "mov", "avi"],
            key="aluno_video_uploader",  # Chave usada nos testes
        )
    ...
    with col2:
        ...
        uploaded_file_mestre = st.file_uploader(
            ...
            type=["mp4", "mov", "avi"],
            key="mestre_video_uploader",  # Chave usada nos testes
        )
    ...
    if st.button(
        "Analisar Movimentos", disabled=analyze_button_disabled, key="analyze_button" # Chave usada nos testes
    ):
        st.info(
    ...
    if (
        ...
        st.session_state["current_frame_aluno_index"] = st.slider(
            ...
            key="aluno_frame_slider",  # Chave usada nos testes
        )
        st.session_state["current_frame_mestre_index"] = st.slider(
            ...
            key="mestre_frame_slider",  # Chave usada nos testes
        )
